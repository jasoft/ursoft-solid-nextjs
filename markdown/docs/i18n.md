...
### 4.1 精确路由匹配
为了节省 Cloudflare Workers 的免费额度（100,000 次/天），**禁止**在生产环境中使用 `www.ursoftware.com/*` 通配符。
- **原因**：通配符会导致软件升级检查（如 `/yuupdate7.inf`）、安装包下载（如 `/dlds/*.exe`）和所有图片资源请求都触发 Worker，导致额度迅速耗尽。
- **方案**：在 `wrangler.toml` 中显式列出需要拦截的路径：
  - 根路径 `/`
  - 所有语言前缀路径（如 `/zh/*`, `/ko/*`）
  - 所有需要重定向的旧路径（如 `/support*`, `/order*`）
